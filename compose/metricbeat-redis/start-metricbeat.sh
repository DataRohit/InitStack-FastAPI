...
echo "Sleeping for 60 seconds before starting Redis Metricbeat..."
sleep 60
...
echo "Setting up Redis Metricbeat user..."
curl -s -X POST -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} "${ELASTICSEARCH_HOSTS}/_security/user/${METRICBEAT_USERNAME}" \
    ...
    -d "{
        ...
        \"full_name\": \"Redis Metricbeat Service User\",
        \"email\": \"metricbeat-redis@initstack.local\"
...
echo "Starting Redis Metricbeat..."
