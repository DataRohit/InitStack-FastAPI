...
echo "Sleeping for 60 seconds before starting System Metricbeat..."
sleep 60
...
echo "Setting up System Metricbeat user..."
curl -s -X POST -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} "${ELASTICSEARCH_HOSTS}/_security/user/${METRICBEAT_USERNAME}" \
    ...
    -d "{
        ...
        \"full_name\": \"System Metricbeat Service User\",
        \"email\": \"metricbeat-system@initstack.local\"
...
echo "Starting System Metricbeat..."
