...
echo "Starting Metricbeat for PostgreSQL monitoring..."
...
sed -i "s|\${POSTGRESQL_HOST}|${POSTGRESQL_HOST}|g" /tmp/metricbeat.yml
sed -i "s|\${POSTGRESQL_PORT}|${POSTGRESQL_PORT}|g" /tmp/metricbeat.yml
sed -i "s|\${POSTGRESQL_USERNAME}|${POSTGRESQL_USERNAME}|g" /tmp/metricbeat.yml
sed -i "s|\${POSTGRESQL_PASSWORD}|${POSTGRESQL_PASSWORD}|g" /tmp/metricbeat.yml
sed -i "s|\${POSTGRES_DB}|${POSTGRES_DB}|g" /tmp/metricbeat.yml
...
until timeout 5 bash -c "</dev/tcp/${POSTGRESQL_HOST}/${POSTGRESQL_PORT}" > /dev/null 2>&1; do
