...
echo "Sleeping for 60 seconds before starting RabbitMQ Metricbeat..."
sleep 60
...
echo "Setting up RabbitMQ Metricbeat user..."
curl -s -X POST -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} "${ELASTICSEARCH_HOSTS}/_security/user/${METRICBEAT_USERNAME}" \
    ...
    -d "{
        ...
        \"full_name\": \"RabbitMQ Metricbeat Service User\",
        \"email\": \"metricbeat-rabbitmq@initstack.local\"
...
echo "Starting RabbitMQ Metricbeat..."
