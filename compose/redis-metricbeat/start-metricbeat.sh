...
echo "Starting Metricbeat for Redis monitoring..."
...
sed -i "s|\${REDIS_HOST}|${REDIS_HOST}|g" /tmp/metricbeat.yml
sed -i "s|\${REDIS_PORT}|${REDIS_PORT}|g" /tmp/metricbeat.yml
sed -i "s|\${REDIS_PASSWORD}|${REDIS_PASSWORD}|g" /tmp/metricbeat.yml
...
until timeout 5 bash -c "</dev/tcp/${REDIS_HOST}/${REDIS_PORT}" > /dev/null 2>&1; do
