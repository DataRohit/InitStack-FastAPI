...
echo "Sleeping for 60 seconds before starting Elasticsearch Metricbeat..."
sleep 60
...
echo "Setting up Elasticsearch Metricbeat user..."
curl -s -X POST -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} "${ELASTICSEARCH_HOSTS}/_security/user/${METRICBEAT_USERNAME}" \
    ...
    -d "{
        ...
        \"full_name\": \"Elasticsearch Metricbeat Service User\",
        \"email\": \"metricbeat-elasticsearch@initstack.local\"
...
echo "Starting Elasticsearch Metricbeat..."
