...
echo "Sleeping for 60 seconds before starting Kibana Metricbeat..."
sleep 60
...
echo "Setting up Kibana Metricbeat user..."
curl -s -X POST -u ${ELASTICSEARCH_USERNAME}:${ELASTICSEARCH_PASSWORD} "${ELASTICSEARCH_HOSTS}/_security/user/${METRICBEAT_USERNAME}" \
    ...
    -d "{
        ...
        \"full_name\": \"Kibana Metricbeat Service User\",
        \"email\": \"metricbeat-kibana@initstack.local\"
...
echo "Starting Kibana Metricbeat..."
