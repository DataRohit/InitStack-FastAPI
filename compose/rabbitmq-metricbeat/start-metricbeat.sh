...
echo "Starting Metricbeat for RabbitMQ monitoring..."
...
sed -i "s|\${RABBITMQ_HOST}|${RABBITMQ_HOST}|g" /tmp/metricbeat.yml
sed -i "s|\${RABBITMQ_PORT}|${RABBITMQ_PORT}|g" /tmp/metricbeat.yml
sed -i "s|\${RABBITMQ_USERNAME}|${RABBITMQ_USERNAME}|g" /tmp/metricbeat.yml
sed -i "s|\${RABBITMQ_PASSWORD}|${RABBITMQ_PASSWORD}|g" /tmp/metricbeat.yml
sed -i "s|\${RABBITMQ_VHOST}|${RABBITMQ_VHOST}|g" /tmp/metricbeat.yml
...
until curl -s "http://${RABBITMQ_HOST}:${RABBITMQ_PORT}/api/overview" -u "${RABBITMQ_USERNAME}:${RABBITMQ_PASSWORD}" > /dev/null; do
