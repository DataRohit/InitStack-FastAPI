...
echo "Starting Metricbeat for SonarQube monitoring..."
...
sed -i "s|\${SONARQUBE_HOST}|${SONARQUBE_HOST}|g" /tmp/metricbeat.yml
sed -i "s|\${SONARQUBE_PORT}|${SONARQUBE_PORT}|g" /tmp/metricbeat.yml
...
until curl -s "http://${SONARQUBE_HOST}:${SONARQUBE_PORT}/api/system/status" > /dev/null; do
    echo "SonarQube is unavailable - sleeping"
    sleep 60
done
